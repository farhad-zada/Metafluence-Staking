...
//0x54d6dBb2E21B924ef902c4427b2ee994765231a0
...
contract StakingSuperPoolUp is
    Initializable,
    IStakeableSuperPool,
    OwnableUpgradeable
{
    ...
    modifier StakeAvailable(address _staker, uint256 _amount) {
        require(
            ...
            "staked amount must be great or equal to minimum staking amount"
        );
        require(_stakeStatus == StakeStatus.ACTIVE, "Staking is not active");
        require(
            totalStaked + _amount <= POOL_MAX_SIZE,
            "reached pool max size"
        );
        require(block.timestamp < stakingExpireDate, "missed or expired.");
        require(
            userTotalStakedAmount(_staker) + _amount <= MAX_STAKING_AMOUNT,
            "reached max staking amount per wallet"
        );
        _;
    }
    ...
    uint constant MAX_STAKING_AMOUNT = 1_000_000_000 * 10 ** 18; //keep max staking amount per wallet

    // wallet infos
    address constant TOKEN_CONTRACT_ADDRESS =
        0xa78775bba7a542F291e5ef7f13C6204E704A90Ba; //Token contract address
    ...
    function stake(
        uint256 _amount
    ) external override StakeAvailable(msg.sender, _amount) {
        Staker memory st = Staker(
            _amount,
            0,
            block.timestamp,
            block.timestamp,
        ...
        if (
            totalStaked >= POOL_MAX_SIZE ||
            POOL_MAX_SIZE - totalStaked < MIN_STAKING_AMOUNT
        ) {
    ...
    function claimAll(uint _id) external override {
        ...
        (
            uint256 finalAmountAfterPenalty,
            uint256 amount
        ) = calculateTransferAmount(msg.sender, _id);
    ...
    function getStakeById(
        address _staker,
        uint _id
    ) internal view returns (Staker memory, uint) {
    ...
    function _calcReward(
        ...
        uint lastRewardableTimestamp = block.timestamp < rewardExpireDate
            ? block.timestamp
            : rewardExpireDate;
        uint _claimedAt = staker.claimedAt;

        uint256 rewardedSeconds = 0;

        if (lastRewardableTimestamp > _claimedAt) {
            rewardedSeconds = lastRewardableTimestamp - _claimedAt;
        }

        uint256 hour = rewardedSeconds / REWARD_DEADLINE_SECONDS;
    ...
    function _calcPenalty(Staker memory request) internal pure returns (uint) {
        return
            request.amount - ((request.amount * PENALTY_PERCENTAGE) / 10 / 100);
    }
